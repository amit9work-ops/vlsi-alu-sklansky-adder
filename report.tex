...
\noindent\textbf{Final sum:} \(\displaystyle S_i = P_i \oplus C_{i-1}\)
...
\node[gr] (l1a) at (2, -1.4) {Gray};
\node[bk] (l1b) at (6, -1.4) {Black};
\draw (pg0) -- (l1a); \draw (pg1) -- (l1a);
\draw (pg2) -- (l1b); \draw (pg3) -- (l1b);

\node[gr] (l2a) at (4, -2.8) {Gray};
\node[gr] (l2b) at (6, -2.8) {Gray};
\draw (l1a) -- (l2a); \draw (pg2) to[out=200,in=90] (l2a);
\draw (l1a) to[out=-20,in=140] (l2b); \draw (l1b) -- (l2b);

\node[font=\scriptsize\color{inksecondary}] at (0,-2.8) {$C_0$};
\node[font=\scriptsize\color{inksecondary}] at (2,-2.8) {$C_1$};
\node[font=\scriptsize\color{inksecondary}] at (4,-3.4) {$C_2$};
\node[font=\scriptsize\color{inksecondary}] at (6,-3.4) {$C_3$};
\draw[-, dashed, inkmuted] (0,-2.55) -- (0,-2.2);
\draw[-, dashed, inkmuted] (2,-2.05) -- (2,-1.75);
\end{tikzpicture}
\caption{Simplified 4-bit Sklansky prefix tree: leaf PG cells generate
per-bit $(P,G)$; two levels of Gray/Black cells combine them into the
group carries $C_1$--$C_3$ in $\log_2(4)=2$ stages. (Conceptual redraw;
the group's actual gate-level netlist is in \S\ref{sec:impl-adder-sub}
and Figure~\ref{fig:floorplan-gatelevel}.)}
...
\addplot[fill=accentblueLight] coordinates {(1.2 V, 4.33)};
\addplot[fill=accentorange] coordinates {(0.9 V, 2.05)};
\end{axis}
\end{tikzpicture}
\caption{Corrected maximum ALU clock frequency vs.\ supply voltage. Lower
$V_{DD}$ roughly halves $f_{max}$, driven by reduced transistor drive
current (see \S\ref{sec:part2-q45} and the $t = C\Delta V / I_{AVG}$
relation below).}
\label{fig:fmax-vdd}
\end{figure}
