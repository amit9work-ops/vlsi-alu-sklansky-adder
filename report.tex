...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=north, text width=12cm, align=left] at ([yshift=-15.3cm]current page.north) {
    ...
    {\normalsize Amit Damari \hfill 326631249}\\
    {\normalsize Tomer Zahuri \hfill 211543277}\\
    {\normalsize Tal Franco \hfill 322402207}\\
    {\normalsize Israel Elmaliach \hfill 206926602}
  };
...
\textbf{Layout utilization:} \(\approx\)97.5\% of the padded (\(+20\%\)) estimate\\[10pt]
\textbf{f\textsubscript{max} @ 1.2\,V:} \textbf{4.33\,GHz} &
\textbf{f\textsubscript{max} @ 0.9\,V:} \textbf{2.05\,GHz} (corrected, see \S\ref{sec:timing})\\[10pt]
...
\bigskip
\noindent\textit{A note on this revision: this report has been re-typeset and
lightly corrected from the group's original submission. A cycle-time
arithmetic slip at the 0.9\,V corner has been fixed (see \S\ref{sec:timing}
for the original vs. corrected numbers), figures have been numbered and
captioned throughout, and one area figure with inconsistent decimal
notation has been normalized and flagged for verification against the
original Virtuoso area report (\S\ref{sec:floorplan}). No underlying design,
schematic, or layout work has changed -- only the write-up.}
...
\[
9\ (\text{326631}\mathbf{249}) + 7\ (\text{211543}\mathbf{277}) +
7\ (\text{322402}\mathbf{207}) + 2\ (\text{206926}\mathbf{602}) = 25
\]
...
\quad (using the same index $i$ as the recurrence above -- e.g.\ $S_1 = P_1 \oplus C_1 = P_1 \oplus G_0$, matching $G_0$ wiring directly into the $S_1$ XOR with no combining cell in Figure~\ref{fig:adder-schematic}, and $S_2 = P_2 \oplus C_2 = P_2 \oplus \text{\sig{out\_gray\_1}}$ one stage later.)\footnote{The original report stated this as $S_i = P_i \oplus C_{i-1}$, which is off by one against its own carry recurrence above; corrected here to match both the recurrence and the actual netlist.}
...
\begin{tikzpicture}[
  ...
  pg/.style={draw=accentblue, fill=accentblue!10, rounded corners=2pt, minimum width=1.0cm, minimum height=0.7cm, font=\scriptsize},
  gr/.style={draw=accentorange, fill=accentorange!12, rounded corners=2pt, minimum width=1.0cm, minimum height=0.7cm, font=\scriptsize},
  bk/.style={draw=inkprimary, fill=inkmuted!18, rounded corners=2pt, minimum width=1.0cm, minimum height=0.7cm, font=\scriptsize},
  ->, >=Latex, accentblue
]
...
\draw[dashed, inkmuted] (pg0.south) -- ++(0,-0.65);
\node[font=\scriptsize\color{inksecondary}] at (0, -1.4) {$C_1{=}G_0$};

\node[gr] (l1a) at (2, -1.4) {Gray}; % G[0:1]
\node[bk] (l1b) at (6, -1.4) {Black}; % (P,G)[2:3]
\draw (pg0) -- (l1a); \draw (pg1) -- (l1a);
\draw (pg2) -- (l1b); \draw (pg3) -- (l1b);

% C2 = G[0:1] -- direct tap off the level-1 Gray cell, one stage deep
\draw[dashed, inkmuted] (l1a.south) -- ++(0,-0.65);
\node[font=\scriptsize\color{inksecondary}] at (2, -2.8) {$C_2{=}G_{0:1}$};

\node[gr] (l2a) at (4, -2.8) {Gray}; % G[0:2] -- combines G[0:1] with PG2
\node[gr] (l2b) at (6, -2.8) {Gray}; % G[0:3] -- combines G[0:1] with (P,G)[2:3]
\draw (l1a) -- (l2a); \draw (pg2) to[out=200,in=90] (l2a);
\draw (l1a) to[out=-20,in=140] (l2b); \draw (l1b) -- (l2b);

\node[font=\scriptsize\color{inksecondary}] at (4,-3.55) {$C_3{=}G_{0:2}$};
\node[font=\scriptsize\color{inksecondary}] at (6,-3.55) {$C_4{=}C_{OUT}$};
\draw[-, dashed, inkmuted] (l2a.south) -- ++(0,-0.35);
\draw[-, dashed, inkmuted] (l2b.south) -- ++(0,-0.35);
\end{tikzpicture}
...
Figure~\ref{fig:adder-schematic}): $C_1{=}G_0$ needs no combining cell at
all (bit 0's generate signal \emph{is} the carry into bit 1); $C_2{=}G_{0:1}$
comes from a single Gray cell; $C_3$ and $C_4$ (the final carry-out) each
need a second stage, so the tree's worst-case depth is 2 -- consistent with
$\lceil\log_2(4)\rceil=2$. Note the fan-out: the level-1 Gray cell's output
feeds \emph{both} level-2 cells, which is what keeps this tree
logarithmic-depth rather than linear. (Conceptual redraw; the group's
actual gate-level netlist is in \S\ref{sec:impl-adder-sub} and
Figure~\ref{fig:floorplan-gatelevel}.)}
...
account for routing between blocks. Once the layout was completed, the
measured utilized area came out slightly lower than that padded estimate --
\textbf{$\approx$97.5\%} of it -- because several VSS power rails ended up
shared between adjacent blocks rather than duplicated per block.

\emph{Note on this figure: the original report gave this measured area as
``50,963.8\,\textmu m\textsuperscript{2}'', using a decimal comma inconsistent
with the rest of the document and off by roughly 1000$\times$ from the scale
established by the 52.288\,\textmu m\textsuperscript{2} gate total one line
above it. Read literally as a decimal-point placement error, the intended
figure is almost certainly \textbf{$\approx$50.96\,\textmu m\textsuperscript{2}}
(consistent with ``slightly lower'' than the padded estimate) -- this is the
value used above, but it should be checked against the original Virtuoso
area report before being treated as final.}
...
distinct delay values} need to be characterized. (The original report's
wording here -- ``there exist 4 different paths,'' followed by three
bullets and a stray, untranslated annotation -- meant exactly this; it has
been spelled out explicitly above.)
...
\textbf{At 1.2\,V:}
\[
t_{cq} + t_{sub,max} = \tfrac{1}{2}\left(0.209\,\mathrm{ns} + 0.2143\,\mathrm{ns}\right) = 0.211\,\mathrm{ns}
\]
...
\textbf{At 1.2\,V:}
\[
T_{cyc} \ge T_{setup} + t_{cq} + t_{sub,max} = 20\,\mathrm{ps} + 0.211\,\mathrm{ns} = 0.231\,\mathrm{ns}
\ \Rightarrow\ f_{max} \approx 4.33\,\mathrm{GHz}
\]

\medskip
\begin{center}
\begin{tikzpicture}
\node[draw=accentorange, line width=1pt, fill=accentorange!8, rounded corners=3pt,
      inner sep=10pt, text width=15cm] {
\begin{minipage}{15cm}
\raggedright
\textbf{\color{accentorange}Correction (0.9\,V corner):} the original
report computed
$T_{cyc} \ge T_{setup} + t_{cq} + t_{sub,max} = 44\,\mathrm{ps} + 0.444\,\mathrm{ns} = 0.444\,\mathrm{ns}$
-- but $44\,\mathrm{ps} + 444\,\mathrm{ps} = 488\,\mathrm{ps}$, not $444\,\mathrm{ps}$; the
setup time was dropped from the sum. The corrected cycle time and
frequency are used below.
\end{minipage}
};
\end{tikzpicture}
\end{center}
...
\addplot+[fill=accentblue,
  ...
  coordinates {(1.2 V,211) [211 ps] (0.9 V,444) [444 ps]};
...
\node[font=\bfseries\large, color=accentorange] at (axis cs:1.2 V,340) {4.33 GHz};
\node[font=\scriptsize, color=inksecondary] at (axis cs:1.2 V,295) {$T_{cyc}=231$\,ps};
...
\fig{alu_full_sim_4p33ghz}{0.92}{Full ALU simulated at the 1.2\,V $f_{max}$ (4.33\,GHz, $T_{cyc}=0.21$\,ns): $A$, $B$, $X$, $C$, and $Y$ tracking correctly cycle-to-cycle.}
...
measured $f_{max}$ of \textbf{4.33\,GHz at 1.2\,V}, degrading to a
corrected \textbf{2.05\,GHz at 0.9\,V} as reduced drive current lengthens
