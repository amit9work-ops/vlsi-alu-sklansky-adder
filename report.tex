...
\textbf{f\textsubscript{max} @ 1.2\,V:} \textbf{4.27\,GHz} &
\textbf{f\textsubscript{max} @ 0.9\,V:} \textbf{2.04\,GHz}\\[10pt]
...
\draw[wire] (busJ) -- (4.5,-4.2) -- (5.82,-4.2) arc (180:0:0.18) -- (6.7,-4.2) -- (6.7,-6.6) -- (l2b.west);
...
The two cursor pairs below sit on CLK edges a half-period apart, so they
capture \emph{opposite-polarity} transitions of $COUT$ (one rising, one
falling) -- not repeated samples of the same event. Rise and fall delays
differ because pull-up/pull-down drive strength isn't symmetric, so the
cycle time must accommodate whichever transition is slower: we take the
\textbf{worst case (maximum)} of the two measured delays, not their
average, since correctness on every cycle can't depend on which
transition happened to occur.

\textbf{At 1.2\,V:}
\[
t_{cq} + t_{sub,max} = \max\left(0.2029\,\mathrm{ns},\ 0.2143\,\mathrm{ns}\right) = 0.2143\,\mathrm{ns}
\]
...
the 0.2029/0.2143\,ns rise/fall delays; the worst case (0.2143\,ns) sets
the cycle time above.}
...
0.4431/0.4453\,ns rise/fall delays; the worst case (0.4453\,ns) sets the
cycle time below.}
\[
t_{cq} + t_{sub,max} = \max\left(0.4431\,\mathrm{ns},\ 0.4453\,\mathrm{ns}\right) = 0.4453\,\mathrm{ns}
\]
...
\textbf{At 1.2\,V:}
\[
T_{cyc} \ge T_{setup} + t_{cq} + t_{sub,max} = 20\,\mathrm{ps} + 0.2143\,\mathrm{ns} = 0.2343\,\mathrm{ns}
\ \Rightarrow\ f_{max} \approx 4.27\,\mathrm{GHz}
\]

\textbf{At 0.9\,V:}
\[
T_{cyc} \ge T_{setup} + t_{cq} + t_{sub,max} = 44\,\mathrm{ps} + 0.4453\,\mathrm{ns} = 0.4893\,\mathrm{ns}
\ \Rightarrow\ f_{max} \approx 2.04\,\mathrm{GHz}
\]
...
\addplot+[fill=accentblue,
  ...
  coordinates {(1.2 V,214) [214 ps] (0.9 V,445) [445 ps]};
...
\node[font=\bfseries\large, color=accentorange] at (axis cs:1.2 V,340) {4.27 GHz};
\node[font=\scriptsize, color=inksecondary] at (axis cs:1.2 V,295) {$T_{cyc}=234$\,ps};
\node[font=\bfseries\large, color=accentorange] at (axis cs:0.9 V,540) {2.04 GHz};
\node[font=\scriptsize, color=inksecondary] at (axis cs:0.9 V,495) {$T_{cyc}=489$\,ps};
\end{axis}
\end{tikzpicture}
...
\fig{alu_full_sim_4p33ghz}{0.92}{Full ALU simulated at 1.2\,V near $f_{max}$ (4.33\,GHz target, $T_{cyc}\approx0.231$\,ns; the corrected worst-case $f_{max}$ from \S\ref{sec:fmax} is 4.27\,GHz): $A$, $B$, $X$, $C$, and $Y$ tracking correctly cycle-to-cycle.}
...
measured $f_{max}$ of \textbf{4.27\,GHz at 1.2\,V}, degrading to
\textbf{2.04\,GHz at 0.9\,V} as reduced drive current lengthens
